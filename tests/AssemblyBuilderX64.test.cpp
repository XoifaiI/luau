...
TEST_CASE_FIXTURE(AssemblyBuilderX64Fixture, "AVXTernaryInstructionForms")
{
    ...
    // packed 32-bit integer arithmetic and logic
    SINGLE_COMPARE(vpaddd(xmm1, xmm2, xmm3), 0xc4, 0xe1, 0x69, 0xfe, 0xcb);
    SINGLE_COMPARE(vpsubd(xmm1, xmm2, xmm3), 0xc4, 0xe1, 0x69, 0xfa, 0xcb);
    SINGLE_COMPARE(vpand(xmm1, xmm2, xmm3), 0xc4, 0xe1, 0x69, 0xdb, 0xcb);
    SINGLE_COMPARE(vpandn(xmm1, xmm2, xmm3), 0xc4, 0xe1, 0x69, 0xdf, 0xcb);
    SINGLE_COMPARE(vpor(xmm1, xmm2, xmm3), 0xc4, 0xe1, 0x69, 0xeb, 0xcb);
    SINGLE_COMPARE(vpxor(xmm1, xmm2, xmm3), 0xc4, 0xe1, 0x69, 0xef, 0xcb);
    SINGLE_COMPARE(vpxor(xmm0, xmm1, xmm2), 0xc4, 0xe1, 0x71, 0xef, 0xc2);
    SINGLE_COMPARE(vpaddd(xmm13, xmm14, xmm15), 0xc4, 0x41, 0x09, 0xfe, 0xef);
    SINGLE_COMPARE(vpaddd(xmm7, xmm12, xmmword[rcx + r10]), 0xc4, 0xa1, 0x19, 0xfe, 0x3c, 0x11);

    // packed 32-bit shifts by immediate (/digit encoding)
    SINGLE_COMPARE(vpslld(xmm5, xmm6, 7), 0xc4, 0xe1, 0x51, 0x72, 0xf6, 0x07);
    SINGLE_COMPARE(vpsrld(xmm5, xmm6, 25), 0xc4, 0xe1, 0x51, 0x72, 0xd6, 0x19);
    SINGLE_COMPARE(vpsrad(xmm5, xmm6, 3), 0xc4, 0xe1, 0x51, 0x72, 0xe6, 0x03);
    SINGLE_COMPARE(vpslld(xmm9, xmm10, 12), 0xc4, 0xc1, 0x31, 0x72, 0xf2, 0x0c);
