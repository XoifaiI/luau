...
declare buffer: {
    ...
    bxor: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number?, count: number?) -> (),
    ...
    add: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number?, count: number?) -> (),
    sub: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number?, count: number?) -> (),
    mul: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number?, count: number?) -> (),
    shl: @checked (b: buffer, offset: number, amount: number, count: number?) -> (),
    shr: @checked (b: buffer, offset: number, amount: number, count: number?) -> (),
    rotl: @checked (b: buffer, offset: number, amount: number, count: number?) -> (),
    fadd: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number?, count: number?) -> (),
    fsub: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number?, count: number?) -> (),
    fmul: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number?, count: number?) -> (),
    fdiv: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number?, count: number?) -> (),
    fmin: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number?, count: number?) -> (),
    fmax: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number?, count: number?) -> (),
    fma: @checked (target: buffer, targetOffset: number, mul: buffer, mulOffset: number?, add: buffer, addOffset: number?, count: number?) -> (),
    transpose: @checked (target: buffer, targetOffset: number, source: buffer, sourceOffset: number, rows: number, columns: number) -> (),
    transposexor: @checked (target: buffer, targetOffset: number, data: buffer, dataOffset: number, source: buffer, sourceOffset: number, rows: number, columns: number) -> (),
    ...
    readu32x8: @checked (b: buffer, offset: number) -> simd256,
    writeu32x8: @checked (b: buffer, offset: number, value: simd256) -> (),
...
-- simd values are opaque lane bundles produced and consumed by the simd library. 128-bit (4-lane, `simd`) and
-- 256-bit (8-lane, `simd256`) are DISTINCT nominal types so the type checker rejects passing a 256-bit value to a
-- 128-bit op (which would silently drop lanes 4..7) and vice-versa. Same boxed runtime object + LUA_TSIMD tag; the
-- split is front-end only. u32x4/f32x4 share `simd`, u32x8/f32x8 share `simd256` (so tofloat/toint reinterpret is legal).
declare extern type simd with
end

declare extern type simd256 with
end
...
declare simd: {
    ...
    unpack: @checked (v: simd) -> (number, number, number, number),
...
-- 8-wide (256-bit) integer lane ops; values use the distinct opaque simd256 type (same boxed object, all 8 lanes)
declare simd256: {
    create: @checked (a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number) -> simd256,
    splat: @checked (v: number) -> simd256,
    extract: @checked (v: simd256, index: number) -> number,
    unpack: @checked (v: simd256) -> (number, number, number, number, number, number, number, number),
    add: @checked (a: simd256, b: simd256) -> simd256,
    sub: @checked (a: simd256, b: simd256) -> simd256,
    mul: @checked (a: simd256, b: simd256) -> simd256,
    band: @checked (a: simd256, b: simd256) -> simd256,
    bor: @checked (a: simd256, b: simd256) -> simd256,
    bxor: @checked (a: simd256, b: simd256) -> simd256,
    bnot: @checked (a: simd256) -> simd256,
    shl: @checked (a: simd256, count: number) -> simd256,
    shr: @checked (a: simd256, count: number) -> simd256,
    rotl: @checked (a: simd256, count: number) -> simd256,
    shuffle: @checked (v: simd256, control: number) -> simd256,
    fadd: @checked (a: simd256, b: simd256) -> simd256,
    fsub: @checked (a: simd256, b: simd256) -> simd256,
    fmul: @checked (a: simd256, b: simd256) -> simd256,
    fdiv: @checked (a: simd256, b: simd256) -> simd256,
    fmin: @checked (a: simd256, b: simd256) -> simd256,
    fmax: @checked (a: simd256, b: simd256) -> simd256,
    fsqrt: @checked (a: simd256) -> simd256,
    fma: @checked (a: simd256, b: simd256, c: simd256) -> simd256,
    tofloat: @checked (a: simd256) -> simd256,
    toint: @checked (a: simd256) -> simd256,
    fsplat: @checked (v: number) -> simd256,
    feq: @checked (a: simd256, b: simd256) -> simd256,
    flt: @checked (a: simd256, b: simd256) -> simd256,
    fgt: @checked (a: simd256, b: simd256) -> simd256,
    select: @checked (mask: simd256, a: simd256, b: simd256) -> simd256,
    eq: @checked (a: simd256, b: simd256) -> simd256,
    lt: @checked (a: simd256, b: simd256) -> simd256,
    gt: @checked (a: simd256, b: simd256) -> simd256,
    sum: @checked (v: simd256) -> number,
    hmin: @checked (v: simd256) -> number,
    hmax: @checked (v: simd256) -> number,
    hband: @checked (v: simd256) -> number,
    hbor: @checked (v: simd256) -> number,
    hbxor: @checked (v: simd256) -> number,
    fsum: @checked (v: simd256) -> number,
    fhmin: @checked (v: simd256) -> number,
    fhmax: @checked (v: simd256) -> number,
}
...
declare f32x8: {
    ...
    sub: @checked (a: simd256, b: simd256) -> simd256,
    mul: @checked (a: simd256, b: simd256) -> simd256,
    div: @checked (a: simd256, b: simd256) -> simd256,
    min: @checked (a: simd256, b: simd256) -> simd256,
    max: @checked (a: simd256, b: simd256) -> simd256,
    sqrt: @checked (a: simd256) -> simd256,
    fma: @checked (a: simd256, b: simd256, c: simd256) -> simd256,
    toint: @checked (a: simd256) -> simd256,
    eq: @checked (a: simd256, b: simd256) -> simd256,
    lt: @checked (a: simd256, b: simd256) -> simd256,
